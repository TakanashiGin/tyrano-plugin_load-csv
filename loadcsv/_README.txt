...
◆ [loadcsv] 独自キーイベント登録
    ...
    to_object  trueを指定すると、key_num属性で指定された列の文字列を各データのキーとして読みこんだCSVデータをオブジェクトに変換します。
                この時、各データのキー名は key_num属性で指定された列数の文字列となります。
                またデータ内のキー名は、CSVデータの 0 行目のデータとなります。
                デフォルトはfalse。

    key  オブジェクト変換時、各データのキー名として扱う列数を指定。デフォルトは 0 。この時、キー名として扱う文字列には半角英数字・文字列の先頭が数字ではないものを推奨します。
